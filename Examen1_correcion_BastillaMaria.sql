...
alter table asignatura modify tipo enum('basica','obligatoria','optativa') not null;-
...
FROM alumno a
WHERE a.id IN (
    SELECT DISTINCT ama.id_alumno
    FROM alumno_se_matricula_asignatura ama
    INNER JOIN asignatura asi ON ama.id_asignatura = asi.id
    WHERE asi.id_grado = 1 ; 
...
WHERE id_grado = (
    SELECT id
    FROM grado
    WHERE nombre = 'Ingeniería Informática' AND nombre_plan = 'Plan 2015'
);
...
SELECT asi.nombre AS nombre_asignatura, ce.anyo_inicio, ce.anyo_fin
FROM alumno_se_matricula_asignatura ama
INNER JOIN asignatura asi ON ama.id_asignatura = asi.id
INNER JOIN curso_escolar ce ON ama.id_curso_escolar = ce.id
WHERE ama.nif_alumno = '26902806M';  
...
WHERE g.nombre = 'Ingeniería Informática' AND g.nombre_plan = 'Plan 2015';

-- 6. Devuelve un listado con todos los alumnos que se han matriculado en alguna asignatura durante el curso escolar 2018/2019.
SELECT DISTINCT a.*
FROM alumno a
INNER JOIN alumno_se_matricula_asignatura ama ON a.id = ama.id_alumno
WHERE ama.id_curso_escolar = 1; 
...
SELECT g.nombre AS grado, COUNT(n.id) AS numero_asignaturas
FROM grado g
LEFT JOIN asignatura n ON g.id = n.id_grado
GROUP BY g.nombre
...
SELECT g.nombre AS grado, n.tipo AS tipo_asignatura, SUM(n.creditos) AS total_creditos
FROM grado g
JOIN asignatura n ON g.id = n.id_grado
GROUP BY g.nombre, n.tipo
...
SELECT ce.anyo_inicio AS año_inicio, COUNT(DISTINCT am.id_alumno) AS numero_alumnos
